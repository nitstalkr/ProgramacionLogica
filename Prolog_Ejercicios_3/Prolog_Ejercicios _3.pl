...
precioTotal(L,H,D,CostoTotal) :- transporte(L,CTransporte),
    alojamiento(H,L,CAlojamiento), CostoTotal is CTransporte
    + (CAlojamiento * D).

%Ejercicio 2

horoscopo(aries,21,1,19,4).
...
signo(D, M, S) :-
    horoscopo(S, D1, M1, D2, M2),
    ((M = M1, D >= D1); (M = M2, D =< D2));
    (M1 > M2, ((M = M1, D >= D1); (M = M2, D =< D2))).
